...
//		outPlotsDir_ = "Plots/Summer_2013/" + cut_ + "/2012ABCD/" + pTvtxN + "/";
	...
	if ( EBEEonly_ )
	{
		files1_.push_back("eff_DATA_Cracks.txt");

		files2_.push_back("eff_MC_Cracks.txt");
        
		files3_.push_back("SF_Cracks.txt");

		etaRegionLegend_.push_back(" Crack probes");
	}
	else
	{
		...
		etaRegionLegend_.push_back("0.80 < |#eta| < 1.48");
		etaRegionLegend_.push_back("1.48 < |#eta| < 2.00");
...
//	texLumi_->SetLineWidth(2);
	...
	TText *text = pt_->AddText( 0.01, 0.3, "CMS Preliminary" );
	text = pt_->AddText( 0.7, 0.3, "#sqrt{s} = 13 TeV, L = 2.26 fb^{-1}" );
...
void plotter::setOutFilexPrefix( string outFilesPrefix )
{
   outFilesPrefix_ = outFilesPrefix;
	...
	if ( EBEEonly_ )
	{
		outFiles_.push_back(outFilesPrefix_ + "_DATA_MC_Cracks_" + pTvtxN_);
	}
	else
	{
...
{
   th1f = new TH1F( "TH1F", "TH1F histogram", 100, 0, 81 );
   th1f->SetMinimum(0);
...
{
   smallSFth1f = new TH1F( "Small TH1F", "Small TH1F histogram", 100, 0, 81 );
...
void plotter::plotEff( string extension )
{
   ...
   setOutFilexPrefix( "eff" );
	...
	if ( cut_ == "ID" )
		...
		yAxisLabel_ = "SIP efficiency";
	else if ( cut_ == "IdIsoSip" )
		yAxisLabel_ = "Efficiency";
	else
		yAxisLabel_ = "Efficiency";

	if ( pTvtxN_ == "pT" || pTvtxN_ == "pT2")
